        ...
        try (PrintWriter out = response.getWriter()) {  if(user!=null) {
            ...
            if(rs.next()) {
             ...
             body=body+"<p class=\"hedi\">Send Money</p><hr/>"
                     ...
                     + "<input type=\"number\" step=\"0.01\" name=\"amont\" placeholder=\"Enter amount\" size=\"51\" class=\"tfd\" /><p/>"
             ...
             if(request.getParameter("stat")!=null) {
                 ...
                 float amt=Float.valueOf(request.getParameter("amount"));
                 String mal=request.getParameter("mail");
                 
                 float selfup,clup=0; 
                 
                 if(Float.valueOf(rs.getString("main"))>=amt) {
                     
                     //perform transaction on self
                     selfup=Float.valueOf(rs.getString("main"))-amt;
                     ...
                     while(rs7.next()) {
                         clup=Float.valueOf(rs7.getString("main"))+amt;
                     }
                     ...
                     body=body+"<hr/>Transaction successfully completed<p/>$"+amt+" has been sent to "+mal+""
                             + "<hr/><a href=\"send\"><button class=\"btn2\">Ok Thanks</button></a><hr/>";
