            ...
            if(rs.next()) {
                if(rs.getString("status").equals("approved")) {
                    ...
                    PayerInfo pyr=(PayerInfo)request.getAttribute("payer");
                            ...
                            //Here is my body
                            body=body+"<p class=\"hedi\">Review</p><hr/>"
                                    ...
                                    + "User Full Names: "+pyr.getFirstName()+" "+pyr.getLastName()+"<p/>"
                                    + "User Email: "+pyr.getEmail()+"<hr/>"
                                    + "<button type=\"submit\" class=\"btn\">PayNow</button>"
                                    + "</form><hr/>";
                    
                    //we do the output stream
                    String content=u.head1+hom+u.head2+btn+u.head3+"Paypal Deposit"+u.comph+body+u.foot1+hom+u.foot2;
